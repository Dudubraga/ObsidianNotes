---

kanban-plugin: board

---

## Backlog

- [ ] **UNICAP** - Sprint 5 (Desenvolvimento 2): [[Projeto de Desenvolvimento Ágil e Eficaz]]
- [ ] **UNICAP** - Entrega Final PDAE
- [ ] **UNICAP** - Projeto 2 [[Arquitetura de Sistemas]]
- [ ] **UNICAP** - Projeto 2 [[Computação Gráfica]]
- [ ] **PIBIC** - Estudar Modelo [[OSVM]]
- [ ] **PIBIC** - Implementar [[OSVM]] em Pytorch
- [ ] **PIBIC** - Leitura [[Write Science in Plain English]]
- [ ] **UNICAP** - Projeto [[Paradigmas de Linguagens de Programação]]
- [ ] **UNICAP** - Projeto 2 [[Ciência dos Dados]]


## Ready

- [ ] **PIBIC** - Implementar [[Isolation Forest]] em Pytorch
- [ ] **PIBIC** - Leitura [[Prediction of Motor Failure Time Using An Artificial.pdf]]
- [ ] **PIBIC** - Aprofundar na Matemática [[Isolation Forest]]
- [ ] **Finalizar** [[Lounge do Açaí]]
- [ ] **PIBIC** - Leitura [[Cooling Fan Fault Diagnostics Using Vibrational and Acoustical Analyses.pdf]]
- [ ] **PIBIC** - Leitura [[Forecasting Imminent Failures in Electrical.pdf]]
- [ ] **Finalizar** [[React Testing Library and Jest]]


## In Progress

- [ ] **Queen** - [[Testes API Postman]]
- [ ] **Queen** - Cypress e2e
- [ ] **UNICAP** - [[Padronização do github]]
- [ ] **PIBIC & UNICAP** - Leitura [[Learning from data a short course.pdf]]
- [ ] **UNICAP** - Sprint 4 (Desenvolvimento 1): [[Projeto de Desenvolvimento Ágil e Eficaz]]


## In Review

- [ ] **PIBIC** - Leitura [[A Unified Framework for Replicability of Anomaly Detection in Multiple Time Series.pdf]]
- [ ] **PIBIC** - Leitura [[Challenges and Opportunities of Deep Learning.pdf]]
- [ ] **Queen** - [[Casos de Teste e2e]]
- [ ] **Maratona** - [[Texto de encerramento]]
- [ ] **Queen** - Desenvolvimento Frontend [[Equipes]]


## Done

**Complete**
- [ ] **Queen** - [[Atualizações no Onboarding]]
- [ ] **Maratona** - [[Papelada]]
- [ ] **PIBIC** - Ficha de Frequência Mensal


***

## Archive

- [x] **UNICAP** - Sprint 3 (Design): [[Projeto de Desenvolvimento Ágil e Eficaz]]

%% kanban:settings
```
{"kanban-plugin":"board","list-collapse":[false,false,false,false,false],"show-checkboxes":false,"full-list-lane-width":true,"move-dates":false}
```
%%